...
class TimerViewModel extends ChangeNotifier {
  /// 일반적인 클래스는 생성자를 호출하여 객체(인스턴스)를 생성
  /// 클래스로 객체(인스턴스)를 생성하면(new) 각각 생성되는 instance 는 독립적이다.
  /// 객체지향에서 한개의 객체(인스턴스)를 만들고, 프로젝트 전체에서 공통으로 공유를 해야하는 경우가 있다.
  /// 이때 static 을 사용하기도 하지만, 싱글톤이라는 패턴(기법)을 사용하여
  /// 이미 생성된 객체를 재활용(다시이용)하는 기법이 있다.
  /// static(프로젝트가 start되면 자동으로 객체 생성)에 비해서 필요한 경우만
  /// 객체를 한번 생성하여 재활용한다는 의미에서 static 에 비하여 메모리 효율성을 기대 할 수 있다.
  ...
  Future<int> getTimer() async {
    // 이 클래스에서 만들어신 _prefs  객체를 private 로 사용하기 위해 요구하기
    final SharedPreferences prefes = await _prefs;
    // localStorage(SP)에 TIMER 라는 이름으로 변수가 저장되어 있으면
    // 정수형으로 변환하여 timer 변수에 담아달라
    var timer = prefes.getInt("TIMER");
    if (timer != null) {
  ...
  void setTimer(timer) async {
    ...
    // SP에 TIMER 라는 이름으로 설정된 타이머 시작값 저장하기
